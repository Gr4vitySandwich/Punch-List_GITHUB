...
//Horizontal (X)
if place_meeting(x+(sign(key_hor)*8),y,obj_player){
 
    while !place_meeting(x+(sign(key_hor)*4),y,obj_player){
        x += key_hor;
    }
   
    if place_meeting(x+(sign(key_hor)*8),y+16,obj_player){
        y = y - 4;
    }
...
//Vertical (Y)
if place_meeting(x,y+(sign(key_vert)*8),obj_player){
 
    while !place_meeting(x,y+(sign(key_vert)*4),obj_player){
        y += key_vert;
    }
   
    if place_meeting(x+16,y+(sign(key_vert)*8),obj_player){
        x = x - 4;
    }
